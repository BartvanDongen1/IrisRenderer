...
Texture2D g_textureMetallicRoughness : register(t1);
...
//        float closestDepth = g_shadowMap.Sample(g_sampler, UV).x;;
...
{
    //float4 myColor = color * g_textureBaseColor.Sample(g_sampler, input.uv);
    ...
    //float lightIntensity = AMBIENT_MULT + (diffuse + spec) * SHADOW_MULT;

    //return myColor * lightIntensity;
    
    if (TEST)
    {
        return color * g_textureBaseColor.Sample(g_sampler, input.uv); 
    }
    return color;
}